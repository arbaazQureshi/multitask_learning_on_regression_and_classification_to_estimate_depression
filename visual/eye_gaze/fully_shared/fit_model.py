...
loss_funcs = {'EG_classification' : 'categorical_crossentropy', 'EG_regression' : 'mse'}
loss_weights = {'EG_classification' : 1.0, 'EG_regression' : 1.0}
metrics = {'EG_classification' : 'accuracy', 'EG_regression' :  ['mae']}

model.compile(optimizer='adadelta', loss = loss_funcs, loss_weights = loss_weights, metrics = metrics)
...
while(current_epoch_number <= total_epoch_count):
	...
	MSE_train = hist.history['EG_regression_loss'][0]
	MAE_train = hist.history['EG_regression_mean_absolute_error'][0]
	crossentropy_train = hist.history['EG_classification_loss'][0]
	accuracy_train = hist.history['EG_classification_acc'][0]
	...
	print(MSE_dev, MAE_dev, crossentropy_dev, accuracy_dev, '\n')
	...
	if(MSE_dev < min_MSE_dev):
		...
		with open('MSE_best.txt', 'w') as f:
			...
			f.write('Corresponing training MSE:\t\t' + str(MSE_train) + '\n')
	...
	if(MAE_dev < min_MAE_dev):
		...
		with open('MAE_best.txt', 'w') as f:
			...
			f.write('Corresponing crossentropy:\t\t' + str(crossentropy_dev) + '\n')
			f.write('Corresponing accuracy:\t\t' + str(accuracy_dev) + '\n')
			f.write('Corresponing training MAE:\t\t' + str(MAE_train) + '\n')
			f.write('Epoch number:\t\t' + str(current_epoch_number) + '\n')
	...
	if(crossentropy_dev < min_crossentropy_dev):
		...
		with open('crossentropy_best.txt', 'w') as f:
			...
			f.write('Corresponing MSE:\t\t' + str(MSE_dev) + '\n')
			f.write('Corresponing accuracy:\t\t' + str(accuracy_dev) + '\n')
			f.write('Corresponing training crossentropy:\t\t' + str(crossentropy_train) + '\n')
			f.write('Epoch number:\t\t' + str(current_epoch_number) + '\n')
		
		model.save('best_crossentropy_model.h5')
		print("SAVING CROSSENTROPY BEST!\n\n")

	if((accuracy_dev > max_accuracy_dev) or ((accuracy_dev == max_accuracy_dev) and (corresponding_crossentropy_dev < min_crossentropy_dev)) ):
		
		max_accuracy_dev = accuracy_dev
		corresponding_crossentropy_dev = crossentropy_dev
